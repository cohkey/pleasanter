...
Pleasanter のサイトパッケージ JSON から、対象テーブルの管理画面設定と `SiteSettings` を API 経由で反映するためのブラウザ用ユーティリティです。
...
- `Aggregations`
- `Columns`
- `EditorColumnHash` / `EditorColumns` / `Sections`
- `LinkColumns`
- `HistoryColumns`
- `MoveTargets`
- `Summaries`
...
- `Gantt` / `EnableGantt`
- `BurnDown` / `EnableBurnDown`
- `TimeSeries` / `EnableTimeSeries`
- `Analy`
- `Kamban` / `EnableKamban`
- `ImageLib`
- `Search`
...
- `CreateColumnAccessControls` / `ReadColumnAccessControls` / `UpdateColumnAccessControls`
- `ChangeHistoryList`

`Views`、`Columns`、`Scripts` などの既知キーは、名前や `ColumnName` を使って差分を出します。未定義のキーは値をそのままコピーします。

`runWizard()` では、対象設定を日本語名のチェックボックスで選択できます。表示名は Pleasanter の管理画面タブに合わせて「全般」「ガイド」「サイト画像」「一覧」「フィルタ」「集計」「エディタ」「リンク」「履歴」「移動」「サマリ」「計算式」「プロセス」「状況による制御」「ビュー」「通知」「リマインダー」「インポート」「エクスポート」「カレンダー」「クロス集計」「時系列チャート」「分析チャート」「カンバン」「画像ライブラリ」「検索」「メール」「サイト統合」「スタイル」「スクリプト」「HTML」「サーバスクリプト」「サイトのアクセス制御」「レコードのアクセス制御」「項目のアクセス制御」「変更履歴の一覧」などを使えます。Console から直接 `applySiteSettings()` を呼ぶ場合も、`sections: "ビュー,項目設定,エディタ"` や `sections: "内容、管理コメント、通知"` のように日本語名、半角/全角カンマ、読点、改行区切りを使えます。

「完全同期(all)」を選ぶ、または `sections: "all"` を指定した場合は、サイト本体設定と `SiteSettings` の両方を対象にします。`replace` モードでは、JSON に存在しない `SiteSettings` も削除対象になります。チェックボックスでは JSON に出ていない既知キーも「未設定」として表示します。個別に選んだ場合も、`replace` なら適用先にだけ存在する該当設定を削除対象にできます。
...
`Package.Permissions`、`Package.PermissionIdList`、`Package.Data` は選択候補には表示しますが、`updatesite` では適用しません。現在は dry-run で「未対応」としてスキップします。サイト/レコード/項目アクセス制御のうち `SiteSettings` に含まれるものは対象にできますが、サイトパッケージ最上位の権限情報は別API対応が必要です。
...
通知、リマインダー、インポート/エクスポートなども、サイトパッケージ JSON の `SiteSettings` に含まれていれば `all` で適用対象になります。ただし、レコードデータ、権限一覧、添付ファイル本体など、サイト管理画面の設定や `SiteSettings` の外側にあるサイトパッケージ要素は `updatesite` 経路では対象外です。

コメントは2種類を分けて考えてください。管理画面上のコメント欄はサイト本体の `Comments` として比較候補に表示しますが、Pleasanter の `updatesite` ではそのまま適用できないため、このツールでは `未対応` としてスキップします。レコードに投稿されたコメント本文そのものもサイト管理画面の設定ではないため、このツールでは移行・適用しません。`Comments` という標準項目をエディタ、表示、通知、エクスポートなどの設定内で参照している場合は、それぞれの対象設定を選べば設定内の参照として適用されます。
