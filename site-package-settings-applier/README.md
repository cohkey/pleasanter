...
`runWizard()` では、対象設定を日本語名のチェックボックスで選択できます。表示名は「表示」「項目設定」「エディタ」「通知」「リマインダー」「エクスポート」などで、英語キーを覚える必要はありません。Console から直接 `applySiteSettings()` を呼ぶ場合も、`sections: "表示,項目設定,エディタ"` や `sections: "表示、通知"` のように日本語名、半角/全角カンマ、読点、改行区切りを使えます。

「完全同期(all)」を選ぶ、または `sections: "all"` を指定した場合は、`replace` モードで JSON に存在しない `SiteSettings` も削除対象になります。単に画面上で全チェックしただけの場合は、JSONに含まれる設定だけを更新対象にします。
...
コメントは2種類を分けて考えてください。`Comments` という標準項目をエディタ、表示、通知、エクスポートなどの設定内で参照している場合は、それぞれの対象設定を選べば適用されます。一方で、レコードに投稿されたコメント本文そのものは `SiteSettings` ではないため、このツールでは移行・適用しません。
