...
コメントは2種類を分けて考えてください。管理画面上のコメント欄はサイト本体の `Comments` なので、「管理コメント」または `Site.Comments` を選ぶと適用対象になります。一方で、レコードに投稿されたコメント本文そのものはサイト管理画面の設定ではないため、このツールでは移行・適用しません。`Comments` という標準項目をエディタ、表示、通知、エクスポートなどの設定内で参照している場合は、それぞれの対象設定を選べば設定内の参照として適用されます。
