...
`runWizard()` では、対象設定を日本語名のチェックボックスで選択できます。表示名は「タイトル」「内容」「管理コメント」「表示」「項目設定」「エディタ」「通知」「リマインダー」「エクスポート」などで、英語キーを覚える必要はありません。Console から直接 `applySiteSettings()` を呼ぶ場合も、`sections: "表示,項目設定,エディタ"` や `sections: "内容、管理コメント、通知"` のように日本語名、半角/全角カンマ、読点、改行区切りを使えます。

「完全同期(all)」を選ぶ、または `sections: "all"` を指定した場合は、サイト本体設定と `SiteSettings` の両方を対象にします。`replace` モードでは、JSON に存在しない `SiteSettings` も削除対象になります。単に画面上で全チェックしただけの場合は、JSONに含まれる設定だけを更新対象にします。
...
通知、リマインダー、インポート/エクスポートなども、サイトパッケージ JSON の `SiteSettings` に含まれていれば `all` で適用対象になります。ただし、レコードデータ、権限一覧、添付ファイル本体など、サイト管理画面の設定や `SiteSettings` の外側にあるサイトパッケージ要素はこのツールの対象外です。
